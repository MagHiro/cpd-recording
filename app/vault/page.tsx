    ...
    <>
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold">Your CPD Recording & Materials</h1>
        </div>
        <div className="rounded-full border border-[#d8e1f5] bg-white px-4 py-2 text-sm text-[#4a5f93]">
          {vault.packages.length} package
          {vault.packages.length === 1 ? "" : "s"}
        </div>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <div className="card">
          ...
          <p className="mt-2 text-2xl font-bold">
            {shortMonth(latestPackage?.classDate ?? latestPackage?.createdAt)}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <section className="card">
          ...
          {featuredVideos.length > 0 ? (
            ...
            <p className="mt-4 text-sm text-[#5b6f9f]">
        ...
        <section className="overflow-hidden rounded-3xl bg-[linear-gradient(165deg,#0f45be_0%,#1c64f2_58%,#29b7f0_100%)] p-6 text-white shadow-[0_20px_40px_rgba(20,68,180,0.35)]">
          <p className="inline-flex rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">
            UPDATED
          </p>
          <h2 className="mt-6 text-3xl font-bold leading-tight">
            Secure CPD portal access
          </h2>
          <p className="mt-4 text-sm text-blue-100">
            Access is restricted to the email used for CPD registration. Session
            recordings and materials appear automatically once access is
            activated
          </p>
